...
Modern `pyproject.toml` (PEP 621) — no `setup.py`. Version is dynamic from
`sphinx_quiz.__version__`; bump with `bump2version` (`.bumpversion.cfg`).
Runtime deps mirror `requirements.in`; dev tooling in `requirements-dev.in`
and the `[dev]` extra. CI in `.github/workflows/` (test matrix; publish on
tag via `python -m build`). Check locally:
```bash
.venv/bin/python -m build && .venv/bin/python -m twine check dist/*
.venv/bin/python -m flake8 sphinx_quiz
```
The README cover `images/cover.png` is regenerated by the **global**
`cover-image` skill (`~/.claude/skills/cover-image/`) via Google AI — needs
`GEMINI_API_KEY` (env, never commit); prompt + reference mockups live in
`scripts/`. The global `python-project-scaffold` skill defines this repo's
house layout (Nekmo style).
...
- The live quiz SPA **wedges the Playwright/CDP screenshot backend** (hangs
  after "fonts loaded"; freezing rAF/animations doesn't help). To capture the
  UI, screenshot a faithful **static HTML mockup** instead — static pages
  capture fine.
- Moving/renaming the repo dir breaks the local `.venv` (stale shebangs in
  console scripts like `flake8`/`twine`); recreate it, or call tools as
  `.venv/bin/python -m <tool>`.
- Don't combine `pkill` with critical follow-up commands in one Bash call — it
  can kill the shell (exit 144). Start `http.server` in its own command; a
  full `rm -rf _build` also kills the server serving it.
- History-preserving repo splits need `git-filter-repo`
  (`pip install git-filter-repo`; not a built-in git subcommand).
